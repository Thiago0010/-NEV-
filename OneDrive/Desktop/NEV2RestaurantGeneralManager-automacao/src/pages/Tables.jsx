...
export default function Tables() {
  ...
  const openTable = async (t) => {
    try {
      ...
      const msg = err.response?.data?.detail || err.message || 'Erro desconhecido ao abrir mesa';
      toast({ title: 'Erro ao abrir mesa', description: msg, variant: 'destructive' });
    }
  };
...
function ComandaBody({ table, orders, itemsByOrder, currency, tax, onChanged }) {
  ...
  const close = async () => {
    try {
      ...
      const msg = err.response?.data?.detail || err.message || 'Erro ao fechar comanda';
      toast({ title: 'Erro ao fechar', description: msg, variant: 'destructive' });
    } finally {
      setClosing(false);
      onChanged();
    }
