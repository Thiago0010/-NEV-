...
def sanitize_payment(payment: Dict[str, Any]) -> Dict[str, Any]:
    """Return a sanitized copy of a Mercado Pago payment object.

    Removes or masks all PII and sensitive data:
    - payer.email, payer.first_name, payer.last_name, payer.identification.number (CPF/CNPJ)
    - payer.address (street, number, neighborhood, city, zip_code)
    - card / payment_method (card token, last 4 digits, brand)
    - metadata (may contain internal data)
    - statement_descriptor
    - Any other fields that could contain sensitive information

    Keeps only fields needed for debugging/auditing:
    - id, status, status_detail, date_created, date_approved
    - transaction_amount, currency_id, installments
    - payment_method_id, payment_type_id
    - external_reference (contains restaurant_id and plan)
    - mp_event_id (for idempotency)
    """
    ...
    # Remove payer PII entirely
    if "payer" in sanitized:
        payer = sanitized["payer"]
        if isinstance(payer, dict):
            # Keep only non-sensitive payer fields
            sanitized["payer"] = {
                k: v for k, v in payer.items()
                if k not in (
                    "email", "first_name", "last_name",
                    "identification", "address", "phone",
                    "date_created", "entity_type"
                )
            }
            # If identification exists, keep only the type (not the number)
            if "identification" in payer and isinstance(payer["identification"], dict):
                ident = payer["identification"]
                if "type" in ident:
                    sanitized["payer"]["identification_type"] = ident["type"]

    # Remove card / payment_method sensitive data
    if "card" in sanitized:
        card = sanitized["card"]
        if isinstance(card, dict):
            sanitized["card"] = {
                k: v for k, v in card.items()
                if k not in ("id", "first_six_digits", "last_four_digits",
                             "expiration_month", "expiration_year",
                             "date_created", "date_last_updated",
                             "cardholder")
            }

    if "payment_method" in sanitized:
        pm = sanitized["payment_method"]
        if isinstance(pm, dict):
            sanitized["payment_method"] = {
                k: v for k, v in pm.items()
                if k not in ("id", "token", "card")
            }

    # Remove metadata entirely (may contain internal data)
    sanitized.pop("metadata", None)

    # Remove statement_descriptor
    sanitized.pop("statement_descriptor", None)

    # Remove additional_info if it contains items with sensitive data
    if "additional_info" in sanitized:
        ai = sanitized["additional_info"]
        if isinstance(ai, dict):
            # Keep only non-sensitive additional_info
            sanitized["additional_info"] = {
                k: v for k, v in ai.items()
                if k not in ("items", "ip_address", "payer")
            }

    # Remove binary_data if present
    sanitized.pop("binary_data", None)

    # Remove order data if present
...
async def create_checkout_preference(
    ...
    if response.get("status") not in (200, 201):
        # Sanitize response before logging - it may contain init_point with preference token
        safe_response = response.copy()
        if "response" in safe_response and isinstance(safe_response["response"], dict):
            resp = safe_response["response"]
            # Remove sensitive URLs that contain tokens
            resp.pop("init_point", None)
            resp.pop("sandbox_init_point", None)
        logger.error("Mercado Pago preference creation failed: %s", safe_response)
        raise RuntimeError(
...
def verify_webhook_signature(
    ...
    *,
    ...
    return ``False`` and log an error — webhook verification is mandatory
    for production security. Development environments MUST set this secret.
    """
