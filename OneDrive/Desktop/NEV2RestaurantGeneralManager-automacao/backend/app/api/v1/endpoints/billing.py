...
async def mercadopago_webhook(
    ...
    try:
        ...
        logger.info("Mercado Pago webhook payload received (keys: %s)", list(payload.keys()))
    ...
    logger.info("Mercado Pago webhook: payment_id=%s, topic=%s", payment_id, topic)
    ...
    # Signature verification (mandatory - webhook rejected if MP_WEBHOOK_SECRET not set).
    ...
    try:
        ...
        logger.info("Mercado Pago webhook: fetched payment %s: %s", payment_id, sanitize_payment(payment))
    except Exception as exc:  # noqa: BLE001
    ...
    if restaurant_id is None:
        ...
        db.add(
            BillingEvent(
                ...
                payload=json.dumps(sanitize_payment(payment), default=str),
                processed=False,
    ...
    logger.info("Mercado Pago webhook: found restaurant %s (plan_name=%s, plan_status=%s)", restaurant.id, restaurant.plan_name, restaurant.plan_status)
    status_mp = payment.get("status")
    applied = _apply_payment_to_restaurant(restaurant, payment, plan_name_hint)
    logger.info("Mercado Pago webhook: _apply_payment_to_restaurant returned applied=%s", applied)
    db.add(
        BillingEvent(
    ...
    await db.commit()
    logger.info("Mercado Pago webhook: committed changes for payment %s", payment_id)
